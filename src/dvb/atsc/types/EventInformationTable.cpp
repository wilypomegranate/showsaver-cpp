...
std::uint16_t
EventInformationSection::init(const std::vector<unsigned char>& buffer) {
...
std::uint16_t
EventInformationTable::init(const std::vector<unsigned char>& buffer,
                            std::uint16_t size) {
  std::uint16_t parsed_bytes = 0;
  ...
  // std::cout << dvb::utils::validate_crc_32(buffer.data(), parsed_bytes, crc_)
  // << std::endl;

  return parsed_bytes;
}

std::uint16_t EventInformationTable::add_event_section(
